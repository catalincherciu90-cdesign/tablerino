...
const _mn = {
    ...
    selectCatHint:     'Selectează o categorie din stânga.',
    noProducts:        'Niciun produs adăugat.',
    available:         'Disponibil',
    unavailable:       'Indisponibil',
    changePic:         'Schimbă poza',
    photoSaved:        'Poză salvată!',
    productAdded:      'Produs adăugat!',
    deleteCatConf:     'Ștergi categoria și toate produsele din ea?',
    deleteProductConf: 'Ștergi produsul?',
    fillNamePrice:     'Completează numele și prețul.',
    error:             'Eroare',
    uploading:         'Se încarcă poza...',
};
...
async function incarcaCategorii() {
    const r = await fetch('/api/admin/categorii');
    ...
    sortableCateg = Sortable.create(list, {
        ...
        onEnd: async () => {
            ...
            await fetch('/api/admin/reordoneaza_categorii', {
                method: 'POST',
...
async function incarcaProduse() {
    if (!catActiva) return;
    const r = await fetch('/api/admin/produse?categorie_id=' + catActiva);
    const d = await r.json();
    ...
    sortableProduse = Sortable.create(list, {
        ...
        onEnd: async () => {
            ...
            await fetch('/api/admin/reordoneaza_produse', {
                method: 'POST',
                headers: {'Content-Type':'application/json'},
...
async function uploadPoza(produsId, input) {
    ...
    try {
        const r = await fetch('/api/upload/poza', { method: 'POST', body: formData });
        const d = await r.json();
...
async function adaugaCategorie() {
    ...
    if (!nume) return;
    await fetch('/api/admin/adauga_categorie', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ actiune: 'adauga_categorie', nume }) });
...
async function stergeCategorie(id) {
    ...
    await fetch('/api/admin/sterge_categorie', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ actiune: 'sterge_categorie', categorie_id: id }) });
...
async function salveazaProdus() {
    ...
    await fetch('/api/admin/adauga_produs', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ actiune: 'adauga_produs', categorie_id: catActiva, nume, descriere: desc, pret }) });
    inchideModal();
    incarcaProduse();
...
async function salveazaEditare() {
    ...
    if (!nume || !pret) { alert(_mn.fillNamePrice); return; }
    const r = await fetch('/api/admin/editeaza_produs', {
        method: 'POST',
        headers: {'Content-Type':'application/json'},
...
async function toggleDisponibil(id) {
    await fetch('/api/admin/toggle_disponibil', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ actiune: 'toggle_disponibil', produs_id: id }) });
    incarcaProduse();
}

async function stergeProdus(id) {
    if (!confirm(_mn.deleteProductConf)) return;
    await fetch('/api/admin/sterge_produs', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ actiune: 'sterge_produs', produs_id: id }) });
    incarcaProduse();
}
